//! Integration tests for `forge-term`.
//!
...
use forge_term::{Color, TermEvent, TermManager};
...
where
    F: FnMut(&forge_term::GridSnapshot) -> bool,
{
...
/// (a) Spawn with cwd=/tmp, write `printf 'hello-forge-term\n'`, and poll
...
async fn spawn_and_capture_output() {
    ...
    manager.write(id, b"printf 'hello-forge-term\\n'\n");

    let found = wait_for_snapshot(&manager, id, Duration::from_secs(10), |snap| {
        snap.contains_text("hello-forge-term")
    })
    .await;
    assert!(found, "expected 'hello-forge-term' to appear in the grid");
