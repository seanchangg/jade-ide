...
use jade_ai::{AiModelId, AiState, InfillRequest, InlineCompletionBackend, ModelSource};
...
fn sprite_tier_serves_a_local_gguf() {
    let _guard = env_lock();
    std::env::set_var("JADE_SPRITE_MODEL", "/tmp/whatever/sprite.gguf");
    assert_eq!(
        AiModelId::Sprite.source(),
        ModelSource::Local(std::path::PathBuf::from("/tmp/whatever/sprite.gguf")),
        "JADE_SPRITE_MODEL redirects the sprite tier"
    );
    std::env::remove_var("JADE_SPRITE_MODEL");
