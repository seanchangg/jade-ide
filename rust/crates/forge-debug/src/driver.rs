...
/// "FORGE_LLDB> "` or on a stale leading prompt, shifting every response by one
/// command. `\nFORGE_LLDB> ` matches only a genuine end-of-output prompt.
...
impl LldbDriver {
    ...
    /// `FORGE_TELEMETRY_SOCK` and the probe dylib (`DYLD_INSERT_LIBRARIES`)
