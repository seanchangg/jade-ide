...
/// A single `__JADE_ALLOC|` / `__JADE_FREE|` line (build-runner.ts `AllocationEvent`).
...
pub enum MemoryEvent {
    ...
    /// `__JADE_HEAP_SUMMARY|` from the malloc interposer.
...
pub struct RunResult {
    pub exit_code: i32,
    pub duration: Duration,
    /// line number → execution count (from `__JADE_TRACE` and/or gcov).
