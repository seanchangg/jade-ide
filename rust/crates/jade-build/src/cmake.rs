...
/// (build-runner.ts:11).
pub const BUILD_DIR_NAME: &str = "cmake-build-jade";
...
pub fn build_cmake_lists(cwd: &Path, active_file: &Path) -> Option<(String, String)> {
    ...
    if let Some(src) = &source_file {
        lines.extend([
            ...
            String::new(),
            "# Jade IDE instrumentation headers (idetools.h), passed at configure time".into(),
            "if(DEFINED JADE_INCLUDE_DIR)".into(),
            format!("    target_include_directories({} PRIVATE ${{JADE_INCLUDE_DIR}})", target),
            "endif()".into(),
            String::new(),
        ]);
...
mod tests {
    ...
    fn generate_for_plain_cpp() {
        ...
        assert!(content.contains("target_include_directories(main PRIVATE ${JADE_INCLUDE_DIR})"));
