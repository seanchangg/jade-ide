...
//! diagnostics, the `__JADE_*` wire lines, AddressSanitizer output, the malloc
...
use jade_telemetry::{Scalar, Timing};
use regex::Regex;
...
// ── `__JADE_*` stdout lines (build-runner.ts:83-143) ──

/// `__JADE_ALLOC|ptr|size|file|line|ts` / `__JADE_FREE|…` (needs ≥6 fields).
pub fn parse_alloc_free(line: &str) -> Option<AllocEvent> {
    let kind = if line.starts_with("__JADE_ALLOC|") {
        AllocKind::Alloc
    } else if line.starts_with("__JADE_FREE|") {
        AllocKind::Free
    } else {
        return None;
    };
...
/// `__JADE_SCALAR|name|step|value|ts` (needs ≥5 fields).
pub fn parse_scalar(line: &str) -> Option<Scalar> {
    if !line.starts_with("__JADE_SCALAR|") {
        return None;
    }
    let parts: Vec<&str> = line.split('|').collect();
    if parts.len() < 5 {
...
/// `__JADE_TIMING|name|duration_ms|step` (needs ≥4 fields).
pub fn parse_timing(line: &str) -> Option<Timing> {
    if !line.starts_with("__JADE_TIMING|") {
        return None;
    }
    let parts: Vec<&str> = line.split('|').collect();
    if parts.len() < 4 {
...
/// `__JADE_HEAP_SUMMARY|total_alloc|total_freed|current_heap|peak_heap|alloc_count|free_count`
...
pub fn parse_heap_summary(line: &str) -> Option<MemoryEvent> {
    if !line.starts_with("__JADE_HEAP_SUMMARY|") {
        return None;
    }
    let parts: Vec<&str> = line.split('|').collect();
    if parts.len() < 7 {
...
Some trailing noise";
    ...
    fn alloc_and_free_lines() {
        let a = parse_alloc_free("__JADE_ALLOC|0xdead|64|main.cpp|10|1234.5").unwrap();
        ...
        let f = parse_alloc_free("__JADE_FREE|0xdead|64|main.cpp|10|1240.0").unwrap();
        assert_eq!(f.kind, AllocKind::Free);
        // too few fields -> None
        assert!(parse_alloc_free("__JADE_ALLOC|0xdead|64").is_none());
        assert!(parse_alloc_free("regular output").is_none());
    }

    #[test]
    fn scalar_and_timing_lines() {
        let s = parse_scalar("__JADE_SCALAR|loss|3|0.125|1000.0").unwrap();
        assert_eq!(s.name, "loss");
        assert_eq!(s.step, 3);
        assert_eq!(s.value, 0.125);
        assert_eq!(s.t, Some(1000.0));
        assert!(parse_scalar("__JADE_SCALAR|loss|3").is_none());

        let t = parse_timing("__JADE_TIMING|forward|12.4|7").unwrap();
        assert_eq!(t.name, "forward");
        assert_eq!(t.ms, 12.4);
        assert_eq!(t.step, 7);
        assert!(parse_timing("__JADE_TIMING|forward").is_none());
    }

    #[test]
    fn heap_summary_line() {
        let e = parse_heap_summary("__JADE_HEAP_SUMMARY|1000|800|200|500|10|8").unwrap();
        match e {
        ...
        assert!(parse_heap_summary("__JADE_HEAP_SUMMARY|1|2|3").is_none());
    }
