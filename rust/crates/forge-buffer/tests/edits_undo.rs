...
use forge_buffer::{Buffer, LspPosition, ManualClock, Selection};

fn manual() -> (Buffer, forge_buffer::ManualClockHandle) {
    let clock = ManualClock::new();
    let handle = clock.handle();
    (Buffer::with_clock("", Box::new(clock)), handle)
}

fn manual_text(text: &str) -> (Buffer, forge_buffer::ManualClockHandle) {
    let clock = ManualClock::new();
    let handle = clock.handle();
