...
//! (`jade-debug`): all mutable protocol state lives in one tokio task, reached
...
/// exactly what the `jade-buffer` crate emits — pass them straight through.
#[derive(Debug, Clone)]
...
impl LspClient {
    ...
    /// * sends `initialized`.
    ///
    /// `include_dir` is the jade include directory (`idetools.h`); when `None`
