//! `forge-build` — the headless build/run/asm engine, a Rust port of
...
//!   `__FORGE_*` protocol handling (§8.2), ASan/gcov parsing, batched memory
//!   events (perf-findings.md #1).
//! - [`BuildEngine::generate_asm`] — `clang++ -S -masm=intel` with `.loc`
//!   source mapping and `c++filt` demangling.
//!
//! Buffer-alias symbolication (`atos`) is provided to `forge-telemetry` via the
//! [`forge_telemetry::Symbolicator`] hook; see [`AtosSymbolicator`] and the
...
pub use forge_telemetry::{Symbolicator, TelemetryServer};
...
pub struct EngineConfig {
    /// `include/` dir with `idetools.h`, passed as `FORGE_INCLUDE_DIR` / `-I`.
    pub forge_include_dir: PathBuf,
    /// `include/forge_interpose.c` (malloc interposer source).
    pub interpose_c: PathBuf,
    /// `probe/forge_probe.mm` (Metal telemetry probe source).
...
impl EngineConfig {
    ...
    pub fn new(forge_include_dir: PathBuf, interpose_c: PathBuf, probe_mm: PathBuf) -> Self {
        EngineConfig {
            forge_include_dir,
            interpose_c,
            probe_mm,
    ...
    /// `<root>/include/forge_interpose.c`, `<root>/probe/forge_probe.mm`.
    pub fn from_repo_root(root: &Path) -> Self {
        EngineConfig::new(
            root.join("include"),
            root.join("include/forge_interpose.c"),
            root.join("probe/forge_probe.mm"),
        )
    }
...
impl BuildEngine {
    ...
    pub async fn generate_asm(&self, file: &Path, extra_flags: &[String]) -> AsmResult {
        asm::generate_asm(file, &self.config.forge_include_dir, extra_flags).await
    }
