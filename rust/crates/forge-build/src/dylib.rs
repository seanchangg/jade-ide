...
pub const INTERPOSE_DYLIB: &str = "/tmp/forge_interpose.dylib";
/// Metal telemetry probe output path (build-runner.ts:27).
pub const PROBE_DYLIB: &str = "/tmp/forge_probe.dylib";
...
/// Ensure `/tmp/forge_probe.dylib` is built and current. Returns `false` on any
...
/// Ensure `/tmp/forge_interpose.dylib` is built and current. Returns `false` on
