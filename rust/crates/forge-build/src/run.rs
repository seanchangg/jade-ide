...
//!
//! Firehose fix (perf-findings.md #1): `__FORGE_ALLOC`/`__FORGE_FREE` lines are
...
async fn run_task(
    ...
    if let Some(tel) = &config.telemetry {
        cmd.env("FORGE_TELEMETRY_SOCK", tel.socket_path());
    }
...
/// Classify one stdout line (build-runner.ts `parseForgeOutput` + the stdout
/// loop). Alloc/free batch; scalar/timing route through telemetry; other
/// `__FORGE_*` lines are swallowed; everything else is program output.
///
/// Deviation: the TS printed *unrecognized* `__FORGE_*` stdout lines as program
/// output (its `parseForgeOutput` returned `false`); we swallow them per the
/// inventory §8.2 ("Unrecognized `__FORGE_*` lines are swallowed, not printed")
/// and the explicit task spec.
fn handle_stdout(
    line: &str,
    batch: &mut Vec<AllocEvent>,
    tx: &mpsc::UnboundedSender<RunEvent>,
    telemetry: Option<&Arc<forge_telemetry::TelemetryServer>>,
) {
    let t = line.trim();
    ...
    } else if t.starts_with("__FORGE_") {
...
/// post-exit ASan parse. Same swallow-unrecognized-`__FORGE_` deviation as
/// [`handle_stdout`].
...
fn handle_stderr(
    ...
    if line.starts_with("__FORGE_TRACE|") {
    ...
    } else if line.starts_with("__FORGE_FUNC_ENTER|") {
        let parts: Vec<&str> = line.split('|').collect();
        if parts.len() >= 3 {
    ...
    } else if line.starts_with("__FORGE_FUNC_EXIT|") {
        *cur_call_depth = cur_call_depth.saturating_sub(1);
    } else if line.starts_with("__FORGE_HEAP_SUMMARY|") {
        if let Some(ev) = parse_heap_summary(line) {
            let _ = tx.send(RunEvent::Memory(ev));
        }
    } else if line.starts_with("__FORGE_INTERPOSE_ACTIVE") {
        // Interposer active marker — nothing to forward.
    } else if line.starts_with("__FORGE_") {
        // Unrecognized instrumentation line — swallow (see doc comment).
...
fn process_gcov(
    ...
    if !executed_lines.is_empty() {
        ...
        let _ = tx.send(RunEvent::Output(format!(
            "\x1b[36m[forge]\x1b[0m Coverage: {} lines executed ({} total hits)\n",
            executed_lines.len(),
            total
        )));
    }
...
mod tests {
    ...
    #[test]
    fn stdout_batches_alloc_free_and_swallows_unknown_forge() {
        let (tx, mut rx) = mpsc::unbounded_channel();
        let mut batch = Vec::new();
        handle_stdout("__FORGE_ALLOC|0x1|8|a.cpp|1|0.0", &mut batch, &tx, None);
        handle_stdout("__FORGE_FREE|0x1|8|a.cpp|1|1.0", &mut batch, &tx, None);
        handle_stdout("hello world", &mut batch, &tx, None);
        handle_stdout("__FORGE_MYSTERY|whatever", &mut batch, &tx, None);
        // alloc/free are buffered, not yet sent; "hello world" is output; the
        // unknown __FORGE_ line is swallowed.
    ...
    fn stdout_flushes_batch_at_cap() {
        ...
        for i in 0..BATCH_CAP {
            handle_stdout(
                &format!("__FORGE_ALLOC|0x{i}|8|a.cpp|1|0.0"),
                &mut batch,
                &tx,
                None,
            );
    ...
    fn stderr_tracks_trace_and_instrumentation() {
        ...
        call("__FORGE_TRACE|f|42|x", &mut executed, &mut funcs, &mut cur, &mut max, &mut san, &tx);
        call("__FORGE_TRACE|f|42|x", &mut executed, &mut funcs, &mut cur, &mut max, &mut san, &tx);
        call("__FORGE_FUNC_ENTER|0xA|f", &mut executed, &mut funcs, &mut cur, &mut max, &mut san, &tx);
        call("__FORGE_FUNC_ENTER|0xB|g", &mut executed, &mut funcs, &mut cur, &mut max, &mut san, &tx);
        call("__FORGE_FUNC_EXIT|0xB", &mut executed, &mut funcs, &mut cur, &mut max, &mut san, &tx);
        call("__FORGE_INTERPOSE_ACTIVE", &mut executed, &mut funcs, &mut cur, &mut max, &mut san, &tx);
        call("__FORGE_HEAP_SUMMARY|1|2|3|4|5|6", &mut executed, &mut funcs, &mut cur, &mut max, &mut san, &tx);
