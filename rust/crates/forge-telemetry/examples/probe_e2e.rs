...
//! `forge_probe.dylib` pointed at this server, enables `model.weights` when it
//! declares, and reports what arrives.
//!
//! Run from the repo root:
//!   cargo run -p forge-telemetry --example probe_e2e -- /path/to/forge-ide/probe
...
async fn main() {
    ...
    let dylib = probe_dir.join("forge_probe.dylib");
    ...
    let sock = std::env::temp_dir().join("forge-telemetry-rust-e2e.sock");
    ...
    let mut child = std::process::Command::new(&train)
        ...
        .env("FORGE_TELEMETRY_SOCK", &sock)
        .env_remove("FORGE_TRACK_ALL") // selection must come from track messages only
